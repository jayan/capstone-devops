...
if [[ "$1" == "devchanged" ]]; then
    ...
    docker tag "react${IMAGE_COUNT}:latest${IMAGE_COUNT}" "${DOCKER_USERNAME}/dev:latest${IMAGE_COUNT}"
    ...
    docker tag "react${IMAGE_COUNT}:latest${IMAGE_COUNT}" "${DOCKER_USERNAME}/prod:latest${IMAGE_COUNT}"
